// #define Status XStatus     // 👈 避免冲突：临时替换 Status 宏
// #include <X11/Xlib.h>      // 👈 先引入 Xlib 并调用 XInitThreads
// #undef Status              // 👈 恢复 Status 宏定义
...
//     XInitThreads();    // 2.必须在任何窗口创建或 OpenCV 使用前调用
...
int main(int argc, char** argv) {
    ...
    video::VideoStream cam1("rtsp://admin:123456@192.168.31.160:554/Streaming/Channels/101", "Cam 1",
                            "/home/ljy/project/poseDetection/config/camera_gp150-160.yaml",
                            detector, pose_estimator, config.getSkeleton());
